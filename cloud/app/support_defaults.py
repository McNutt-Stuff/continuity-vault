...
_SOURCE_PAGES = [
    ...
    _source_doc(
        ...
        extra=(
            ...
            "\n"
            "- **Coverage & freshness** — every in-scope managed source is active and has a recent, "
            "recoverable recovery point; stale or failed sources are named.\n"
            "- **Strong authentication** — MFA registration, phishing-resistant methods, and "
            "privileged/admin MFA coverage.\n"
            "- **Access policy & exposure** — conditional access, external-sharing control, and "
            "guest-account governance.\n"
            "- **Residency** — tenant data location.\n"
            "- **Security posture** — Microsoft Secure Score.\n"
            "- **Privileged access** — Entra admin-role holders.\n"
            "- **Endpoint devices** — Intune device compliance and disk encryption.\n"
            "- **Authentication policy** — strong auth methods enforced, weak SMS/voice disabled.\n"
            "\n"
            "Arkive assesses each of these using the same read-only Microsoft access it already holds — "
            "nothing for you to configure. Anything Microsoft hasn't made available yet is shown as "
            "*needs attention* with a one-click **Re-authorize**, never a silent pass. You can override "
            "any control's state, record a time-boxed exception, and re-assess on demand. Ingest-time "
