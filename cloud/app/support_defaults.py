...
_SOURCE_PAGES = [
    ...
    _source_doc(
        ...
        extra=(
            ...
            "**Settings → Traffic Identification (DPI)** on the UniFi controller; data appears on the "
            "next poll.\n"
            "\n## Mapping devices to people\n"
            "Under **Clients & devices** you can assign each observed device to a specific member of "
            "your household/organization (an evolution of *Me / My Family*). This attributes traffic "
            "to a person so no one's usage is missed.\n"
            "\n## Advanced Ubiquiti Analytics\n"
            "When enabled (a feature your administrator turns on), an **Advanced analytics** tab adds "
            "an org-level per-person rollup and a per-user drilldown — traffic trends, that person's "
            "devices, and their apps filterable by category — plus **collection-gap detection** that "
            "flags missing days or stale devices so you can spot data that's being missed on a "
            "per-person or per-device basis. **Re-map apps** re-runs the app→source registry so newly "
            "supported services are recognised in already-collected traffic.\n"
            "\n## Troubleshooting a stall\n"
            "If data stops updating, check the integration's health badge and last-success time. A "
            "stall usually means the appliance went offline or a re-authentication is pending — "
            "Arkive now alerts you when an integration stops collecting.\n"
            "\n## Availability\n"
            "::: plan business\n"
    ...
    _source_doc(
        ...
        extra=(
            ...
            "\n"
            "> **OneNote is app-only limited.** Microsoft Graph's OneNote API does not reliably accept "
            "application (app-only) tokens — even with `Notes.Read.All` granted it can return "
            "`401 / 40001 (no valid authentication token)`. When that happens Arkive marks the OneNote "
            "sources **unavailable** (a Microsoft limitation, not a missing grant) and stops retrying "
            "them more than once a day, so it won't repeatedly alert. A restricted SharePoint site "
            "(`403 accessDenied`, e.g. under *Sites.Selected*) is likewise marked **unavailable** for "
            "that one site while other sites keep collecting.\n"
            "\n"
            ...
            "\n## Compliance\n"
            "The platform **Compliance** area assesses framework packs (NIST CSF, CIS, ISO 27001, "
            "SOC 2, HIPAA, GDPR) against your live footprint and folds Microsoft 365 in as evidence. "
            "Controls are scored **scope-aware** — a coverage num/den (how many in-scope sources are "
            "actually protected and recoverable), the **evidence level** (observed vs verified test vs "
            "configured), the specific **accounts/systems** dragging a control down, and **freshness** "
            "(stale evidence never counts as met). What Microsoft 365 contributes:\n"
            "\n"
            "- **Coverage & freshness** (no extra permission) — every in-scope managed source is active "
            "and has a recent, recoverable recovery point; stale or failed sources are named.\n"
            "- **Strong authentication** (`Reports.Read.All`/`AuditLog.Read.All`) — MFA registration, "
            "phishing-resistant methods, and privileged/admin MFA coverage.\n"
            "- **Access policy & exposure** (`Policy.Read.All`, `SharePointTenantSettings.Read.All`, "
            ...
            "can override any control's state, record a time-boxed exception, and re-assess on demand. "
            "Full permission reference: `docs/integrations/microsoft365-permissions.md`. Ingest-time "
            "governance (label, restrict, obfuscate, don't-index, discard) is handled separately by the "
            "**Rules** engine, scoped to a managed source or source type.\n"
            "\n### Policy & procedure questionnaire\n"
            "Frameworks also include **organizational controls** — written policies, risk assessments, "
            "security awareness training, incident-response and business-continuity plans, vendor risk, "
            "access reviews, change management, vulnerability management, penetration testing, and "
            "physical/personnel security — that no tool can measure automatically. The **Compliance** "
            "page has a **Policy & procedure questionnaire**: attest to each (in place / partial / not in "
            "place / not applicable), add a note, **attach your proof** (upload the policy document, stored "
            "encrypted, or link a URL), and set a review date. Answers are audited and re-scored immediately; "
            "an attestation past its review "
            "date goes stale and prompts re-attestation, and an un-answered item counts as *not yet "
            "demonstrated* — so a framework score reflects both what Arkive proves automatically and what "
            "you attest.\n"
            "\n## Availability\n"
            "::: plan business\n"
