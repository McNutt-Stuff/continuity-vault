"""Ubiquiti UniFi (Dream Machine / UniFi OS) integration — control-plane spec."""
...
class UbiquitiIntegration(Integration):
    """UniFi controller integration. Runs on the customer appliance (needs LAN
    access) and reports which apps/cloud services are in use, which clients use
    them, and how much traffic — feeding shadow-app detection, analytics and
    insights."""
    ...
    def spec(self) -> IntegrationSpec:
        return IntegrationSpec(
            ...
            version="1.1.0",
            status="ga",
            ownership_models=["user", "organization"],
            docs_slug="integration-ubiquiti",
            credential_fields=[
