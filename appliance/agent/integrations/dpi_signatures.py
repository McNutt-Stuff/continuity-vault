"""UniFi DPI category/application id -> name catalog.

Auto-generated from Ubiquiti's usg-dpi signature data (as published in the
unpoller/unifi project). Do not hand-edit; regenerate from the upstream table.

The v2 traffic API returns numeric ``category`` and ``application`` ids. Resolve
a friendly name with :func:`app_name`:

    * category name  = ``DPI_CATS[category]``
    * application    = ``DPI_APPS[(category << 16) + application]``

(The application map is keyed by the combined ``cat<<16 + app`` value, exactly
as UniFi/ntop encode it — e.g. app 65535 / cat 255 -> 16777215 -> "Unknown_Other".)
"""
...
# (category << 16 | application) -> human name.
...
def app_name(app_id, cat_id) -> tuple[str, str]:
    """Return (application_name, category_name) for the given numeric ids.

    Falls back to the category name, then a generic ``App <id>`` label. App id
    65535 is UniFi's "unidentified traffic" sentinel.
    """
    category = DPI_CATS.get(int(cat_id)) if cat_id is not None else ""
    if app_id in (None, 65535):
        return ("Unidentified" if app_id == 65535 else "Unknown", category or "")
    combo = (int(cat_id) << 16) + int(app_id) if cat_id is not None else None
    name = DPI_APPS.get(combo) if combo is not None else None
    if name and not name.startswith("Unknown"):
        return (name, category or "")
    return (category or f"App {app_id}", category or "")
