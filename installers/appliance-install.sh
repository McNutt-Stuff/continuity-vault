...
# Set the system hostname to the configured name, else the appliance serial
# (generated by the agent, read from /status). Runs after the agent is up so the
# serial exists; idempotent on re-runs / self-updates.
set_appliance_hostname() {
  local name="${CV_HOSTNAME:-}"
  if [[ -z "$name" && -f /etc/continuity-vault/appliance.env ]]; then
    name="$(grep -E '^CVA_HOSTNAME=' /etc/continuity-vault/appliance.env 2>/dev/null | head -1 | cut -d= -f2- | tr -d '"' || true)"
  fi
  if [[ -z "$name" ]]; then
    name="$(curl -fsS "http://127.0.0.1:8090/status" 2>/dev/null \
      | grep -o '"serial": *"[^"]*"' | head -1 \
      | sed 's/.*"serial": *"\([^"]*\)".*/\1/' || true)"
  fi
  if [[ -n "$name" ]]; then
